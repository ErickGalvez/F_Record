...
function updateSavePath() {
    ...
    loadImageCount();
    updateRecordButtonBasedOnImageCount();
}
...
function addFolder() {
    const rootPath = localStorage["rootPath"];
    if (!rootPath) {
        alert("Please set the root path first.");
        return;
    }

    let folderName = "Recording1";
    let folderPath = `${rootPath}/${folderName}`;

    // Loop to ensure unique folder names at the root level
    let counter = 1;
    while (fs.existsSync(folderPath)) {
        counter++;
        folderName = `Recording${counter}`;
    ...
    // Update the save path and reset the recording state
    document.getElementById("savePath").value = folderPath;
    localStorage.setItem("savePath", folderPath);

    // Reset imageCount and update the UI
    document.getElementById("imageCount").innerHTML = "0";
    updateRecordButtonState("StartRecording");

    // Notify the application of the new path
    csInterface.evalScript(`$._ext.sendToGenerator('${JSON.stringify({ from: 'panel', action: 'update-save-path', value: encodeURIComponent(folderPath) })}')`);
}



document.addEventListener("DOMContentLoaded", function () {
    // Initialize existing buttons and UI elements
    initButtons();
    setupFileStructure();
...
function updateRootPath() {
    localStorage["rootPath"] = document.getElementById("rootPath").value;
}
